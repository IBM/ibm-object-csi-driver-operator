...
# entire set of makefiles included in this invocation, looking for lines of the file as xyz: ## something,
# and then pretty-format the target and help.
.PHONY: help
...
test: manifests generate fmt vet envtest ## Run tests and generate coverage report.
	...
	## Run go test with coverage and exclude specific directories like /e2e, /fake, and api/v1alpha1
	go test -coverprofile=coverage.out $(shell go list ./... | grep -v '/e2e' | grep -v '/fake' | grep -v 'api/v1alpha1')
